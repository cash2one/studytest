...
mark_oss::mark_oss(string h, string u, string d, string p, string c, int pt):DB(){
	charset = c;
	port = pt;
	mysql_host = h;
	mysql_user = u;
 ...
 */
int mark_oss::getList(const string& table, const map<string,vector<string> >& where, int page, int pagesize, const string& format_field, const map<string, string>& orderby, map<string, map<string, string> >& ret ){
 ...
 *
 */
int mark_oss::getList(	const string& table, 
						const map<string,vector<string> >& where, 
						int start, 
						int limit, 
 ...
 */
int mark_oss::format_orderby_limit( int page, 
									int pagesize, 
									const map<string, string>& orderby, 
									int limit_type, 
									string& ret)
{
	///先处理 ORDER BY语句
