 ...
 * 获取所有结果
 */
int DB::fetch_all(	const string& sql,  //SQL语句
					const vector<string>& param,  //参数
					map<int,map<string,string> >& ret)
{
	int ret_status = query(sql, param);
	if( 0 != ret_status){
...
int DB::format_where_cond(const map<string,vector<string> >& param, string& where, vector<string>& data){
	...
	for(iter = param.begin(); iter != param.end(); iter++){
		...
		else if(sql_compare_symbol.compare("FIND_IN_SET") == 0){ //find_in_set
...
int DB::bind_param_execute(const string& sql, const vector<string>& param){
    ...
    if(mysql_stmt_prepare(stmt,sql.c_str(),strlen(sql.c_str()) ) !=0){
        fprintf(stderr,"[DB::bind_param_execute] prepare sql failed:%s SQL[%s]\n",mysql_stmt_error(stmt),sql.c_str());
        return -1;
    }
	...
	if(para_num != param.size()){
		fprintf(stderr,"[DB::bind_param_execute] bind param failed SQL[%s]\n",sql.c_str());
		return -2;
	}
    ...
    if(mysql_stmt_bind_param(stmt,bind) != 0){
        fprintf(stderr,"[DB::bind_param_execute] mysql_stmt_bind_param failed:%s SQL[%s]\n", 
				mysql_stmt_error(stmt),sql.c_str());
		free(bind);
		free(param_len);
        return -2;
    ...
    if(mysql_stmt_execute(stmt)!=0){
        fprintf(stderr,"[DB::bind_param_execute] execute failed:%s SQL[%s]\n",
				mysql_stmt_error(stmt),sql.c_str());
		free(bind);
		free(param_len);
        return -2;
