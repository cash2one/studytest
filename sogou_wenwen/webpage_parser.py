...
class WebPageParser:
    ...
    def __ParseElementList__(self):
        ...
        for element in self.page_element_list:
            ...
            if is_hit_item_rule == False:
                ...
                self.__sogou__wenwen__(element)
            print "----------------------print log "
   ...
   #提取sogou问问的聚合信息
    def __sogou__wenwen__(self, element):
        if element.search_result == None:
            element.search_result = webpage.SearchResult()
        abstract = ""
        for child in element.bs_tag.descendants:
            if isinstance(child, NavigableString):
                ...
                if len(child.string.strip()) == 0:
                    continue
                abstract += child.string.strip()
        if abstract.find("搜狗问问") <= 0:
            return
        #Find url and title
        for url_node in element.bs_tag.find_all("a"):
            item = dict({"url":"","title":"","docid":""})
            if len(url_node.text.strip()) == 0:
                continue
            if "href" in url_node.attrs:
                if url_node["href"] == "#": #Not url node
                ...
                continue
            if "id" in url_node.attrs:
                if not url_node["id"].startswith("sogou_vr_30002003"):
                    continue
            else:
                continue
            url = url_node["href"].strip()
            url = common.NormalizeUrl(url, self.config.url_pattern)
            if url.find("wenwen.sogou.com") <= 0:
                continue
            docid = int(common.GetDocID(url), 10)
            item["url"] = url_node["href"].strip()
            item["docid"] = docid
            ...
            for child in url_node.descendants:
                if isinstance(child, NavigableString):
                    if child.parent.name.lower() in webpage.NOT_ANCHOR_ITEMS_NAME:
                        continue
                    item["title"] += child.string.strip()
            if item["title"].find("详情>>") >= 0:
                return
            self.wenwen.append(item)
        return
