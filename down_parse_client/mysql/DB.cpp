...
DB::~DB()
{
	if(NULL != stmt)
	{
		fprintf(stderr,"[DB::~DB] start close mysql statement.........\n");
		mysql_stmt_close(stmt);
	}
	if(NULL != mysql)
	{
		fprintf(stderr,"[DB::~DB] start close mysql connection.........\n");
		mysql_close(mysql);
	}
}
...
int DB::initDB()
{
	if(NULL == mysql)
	{
		...
		{
			fprintf(stderr,"[DB::initDB] connect MySQL failed:%s,exit..............\n",mysql_error(mysql));
			exit(EXIT_FAILURE);
		}
		fprintf(stderr,"[DB::initDB] init mysql connection successfully.........\n");
		string init_sql = "SET NAMES ";
		init_sql += charset;
		mysql_real_query(mysql,  init_sql.c_str(), init_sql.length());
		fprintf(stderr,"[DB::initDB] execute SQL[%s]\n",init_sql.c_str());
	}
	if(NULL == stmt)
	{
		...
		if(!stmt)
		{
			fprintf(stderr,"[DB::initDB] mysql_stmt_init failed,exit............\n");
			exit(EXIT_FAILURE);
		}
		fprintf(stderr,"[DB::initDB] init mysql statement successfully.........\n");
	}
	return 0;
}
...
int DB::fetch_row(const string& sql, const vector<string>& param, map<string,string>& result){
	...
	if( 0 != ret_status)
	{
        fprintf(stderr,"[DB::fetch_row] query failed,sql=%s\n",sql.c_str());
		return -1;
	}
	
	///获取结果
	map<int,map<string,string> > ret;
	int tmp = get_execute_result(ret);
	if(ret.size() <= 0)
	{
        fprintf(stderr,"[DB::fetch_row] get_execute_result failed,ret=%d\tsql=%s\n",tmp,sql.c_str());
		return -2;
	}
	map<int,map<string,string> >::iterator iter;
	iter = ret.find(0);
...
int DB::insert_update(const string& table, const map<string,string>& insert_data, const map<string,string>& update_data)
{
	...
	string sql = "INSERT INTO `";
	sql.append(table).append("` SET ").append(i_sql).append(" ON DUPLICATE KEY UPDATE ").append(u_sql);
...
int DB::format_where_cond(const map<string,vector<string> >& param, string& where, vector<string>& data)
{
	...
	for(iter = param.begin(); iter != param.end(); iter++)
	{
		...
		///依次判断
		if( sql_compare_symbol.compare("IN") == 0)
		{ ///in
		...
		else if(sql_compare_symbol.compare("LIKE") == 0)
		{ //like
...
int DB::bind_param_execute(const string& sql, const vector<string>& param)
{
	...
	{
        fprintf(stderr,"[DB::bind_param_execute] prepare sql failed: errmsg[%s] errno[%u] SQL[%s]\n",
				mysql_stmt_error(stmt),mysql_stmt_errno(stmt),sql.c_str());
        return -1;
    }
	...
	{
		fprintf(stderr,"[DB::bind_param_execute] bind param failed : errmsg[%s] errno[%u] SQL[%s]\n",
				mysql_stmt_error(stmt),mysql_stmt_errno(stmt),sql.c_str());
		return -2;
	}
	...
	{
        fprintf(stderr,"[DB::bind_param_execute] mysql_stmt_bind_param failed: errmsg[%s] errno[%u] SQL[%s]\n", 
				mysql_stmt_error(stmt),mysql_stmt_errno(stmt),sql.c_str());
		free(bind);
		free(param_len);
        return -2;
	...
	{
        fprintf(stderr,"[DB::bind_param_execute] execute failed: errmsg[%s] errno[%u] SQL[%s]\n",
				mysql_stmt_error(stmt),mysql_stmt_errno(stmt),sql.c_str());
		free(bind);
		free(param_len);
        return -2;
...
int DB::get_execute_result(map<int,map<string,string> >& ret)
{
	...
	{
        fprintf(stderr,"[DB::get_execute_result] mysql_stmt_bind_result failed, errmsg[%s]  errno[%u]\n",
				mysql_stmt_error(stmt),mysql_stmt_errno(stmt));
        exit(EXIT_FAILURE);
    }
    if(0 != mysql_stmt_store_result(stmt))
	{
        fprintf(stderr,"[DB::get_execute_result] mysql_stmt_store_result failed, errmsg[%s]  errno[%u]\n",
				mysql_stmt_error(stmt),mysql_stmt_errno(stmt));
        exit(EXIT_FAILURE);
    }
	...
	while( !mysql_stmt_fetch(stmt) )
	{
		...
		for(int j=0; j<column_num; j++)
		{
			...
			{
				fprintf(stderr,"[DB::get_execute_result] mysql_stmt_fetch_column failed, errmsg[%s]  errno[%u]\n",
						mysql_stmt_error(stmt),mysql_stmt_errno(stmt));
				continue;
			}
