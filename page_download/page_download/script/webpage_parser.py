...
class WebPageParser:
    ...
    def ParseResultHTML(self, html_string):
        ...
        self.parser.Reset()
        soup = BeautifulSoup(html_string, "html5lib",from_encoding="gbk")
    ...
    def __ParseElementList__(self):
        ...
        for element in self.page_element_list:
            ...
            #规则抽取
            if len(element.bs_tag.findAll("script")) > 0:   #检查是否包含script节点
        ...
        for element in result_list:
            ...
            if url in redirect_urls:                
                ...
                if not self.config.redirect_url_scheme_re.match(url):                                               
                    ...
                    self.logger.info("GetRedirect: " + url.encode('gbk', 'ignore'))
                    redirect_url = common.GetRedirectURL(url.encode('gbk', 'ignore'))               
                    if redirect_url != "":
    ...
    def RebuildAnchorPage(self):
        for result in self.page_element_list:        
            ...
            soup = BeautifulSoup(head_string, "html5lib",from_encoding="gbk")
            result.bs_tag.insert_before(soup.a)
            tail_string = self.result_anchor_tail_string % (str(result.rank), str(result.index))
            soup = BeautifulSoup(tail_string, "html5lib",from_encoding="gbk")
        ...
        for container_index,container in enumerate(container_list):
            ...
            for element_index,element_node in enumerate(container):
                ...
                if element_node.name.lower() == "style":  #判断element是否js或者css标签
                    element.item_id = webpage.ITEM_ID_STYLE
                    element.item_desc = "style"
                    element.node_info = "style"
                    element.is_eval = False
                    element.is_rank = False                    
                    element.is_save = False                    
                elif element_node.name.lower() == "script":
                    element.item_id = webpage.ITEM_ID_SCRIPT
                    element.item_desc = "script"
                    element.node_info = "script"
                    element.is_eval = False
                    element.is_rank = False
                    element.is_save = False
                elif element_node.name.lower() == "br":
                    element.item_id = webpage.ITEM_ID_BR
                    element.item_desc = "br"
                    element.node_info = "br"
                    element.is_eval = False
                    element.is_rank = False
                    element.is_save = False
                elif element_node.name.lower() == "link":
                    element.item_id = webpage.ITEM_ID_LINK
                    element.item_desc = "link"
                    element.node_info = "link"
                    element.is_eval = False
                    element.is_rank = False
                    element.is_save = False
                elif "style" in element_node.attrs and self.display_none_re.match(element_node.attrs["style"]) and self.config.engine != "sogou":
                    element.item_id = webpage.ITEM_ID_DISPLAY_NONE
                    element.item_desc = "display none"
                    element.node_info = self.parser.GetNodeInfo(element_node, self.config.node_attrs)
                    element.is_eval = False
                    element.is_rank = False
                    element.is_save = False
                elif len(list(element_node.stripped_strings)) == 0 and len(element_node.findAll("iframe")) == 0:
                    element.item_id = webpage.ITEM_ID_NO_TEXT
                    element.item_desc = "no text"
                    element.node_info = self.parser.GetNodeInfo(element_node, self.config.node_attrs)
                    element.is_eval = False
                    element.is_rank = False
                    element.is_save = False                                                      
    ...
    def __HandlePathRule__(self, rule, element):
        ...
        for found_node in found_nodes:
            ...
            if rule.node_name != None and len(rule.node_name) > 0 \
                ...
                continue
            #判断文本是否匹配
            if rule.text_re != None and not rule.text_re.match(found_node.text.strip().encode('gbk','ignore')):
                continue
            #判断结构是否匹配
...
if __name__ == "__main__":
    ...
    for index,result in enumerate(parser.page_element_list):
         ...
         print "url\t",result.search_result.url.encode('utf-8','ignore')
         print "raw_url\t",result.search_result.raw_url.encode('utf-8','ignore')
         print "title\t",result.search_result.title.encode('utf-8','ignore')
