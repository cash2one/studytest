...
class DB(object):
    ...
    def update(self, table, where, data):
        ...
        sql = "UPDATE `" + table + "` SET " + self.buildSQL(",", **data)
        ...
        affected_rows = 0
        try:
            affected_rows = self.execute(sql)
        except Exception, args:
            print args
            return 0
    ...
    def insert_update(self, table, insert_data, update_data):
        ...
        # 拼装SQL
        sql = "INSERT INTO `" + table + "` SET " + self.buildSQL(",", **insert_data)
    ...
    def _getConn(self):
        try:
            ...
            self.conn = MySQLdb.connect(user=self.dbInfo["user"], passwd=self.dbInfo["passwd"], \
                                        host=self.dbInfo["host"], db=self.dbInfo["db"], port=int(self.dbInfo["port"]), \
                                        charset=self.dbInfo["charset"], cursorclass=MySQLdb.cursors.DictCursor)
...
if __name__ == "__main__":
    c = DB(user="phpmyadmin", passwd="123456", host="127.0.0.1", db="db_crawler", port=3306, charset="utf8")
    # print "----------fetch_row-------------"
