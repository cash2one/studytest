...
class BaseParser:
    ...
    def Parse(self, html_string, encoding):
        ...
        if self.soup.declared_html_encoding is None or len(self.soup.declared_html_encoding) <= 0:
            ...
            self.logger.info("declared encoding: "+found_encoding.encode('gbk'))
        #re-encoding html
        new_html_string = html_string.decode(found_encoding, 'ignore').encode(encoding, 'ignore')
        self.logger.info("convert encoding to " + encoding.encode('gbk'))
        self.soup = BeautifulSoup(new_html_string, "html5lib", from_encoding=encoding)
        #remove comments
